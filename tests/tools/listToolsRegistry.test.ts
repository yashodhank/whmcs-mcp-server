...
function harness() {
  ...
  const server = {
    registerTool: (n: string, cfg: unknown, cb: any) => {
      configs[n] = cfg;
      handlers[n] = cb;
    },
  };
...
describe('registerListTools', () => {
  ...
  it('get_activity_log maps WHMCS GetActivityLog entries (newest first)', async () => {
    ...
    expect(read).toHaveBeenCalledWith('GetActivityLog', {
      clientid: 9001,
      limitnum: 10,
      limitstart: 0,
    });
  ...
  it('list_client_services maps WHMCS GetClientsProducts rows', async () => {
    ...
    expect(read).toHaveBeenCalledWith('GetClientsProducts', {
      clientid: 9001,
      limitnum: 2,
      limitstart: 0,
    });
    ...
    expect(p.items[0]).toEqual({
      ...
      next_due_date: '2030-04-14',
      recurring_amount: '3.00',
      payment_method: 'card',
    });
  });
  ...
  it('list_client_domains maps WHMCS GetClientsDomains rows', async () => {
    ...
    expect(read).toHaveBeenCalledWith('GetClientsDomains', {
      clientid: 9001,
      limitnum: 10,
      limitstart: 0,
    });
  ...
  it('list_client_invoices maps WHMCS GetInvoices rows with userid + fixed ordering + status', async () => {
    ...
    expect(read).toHaveBeenCalledWith('GetInvoices', {
      userid: 9001,
      limitnum: 10,
      limitstart: 0,
      orderby: 'date',
  ...
  it('list_client_tickets maps GetTickets rows, sorts by lastreply DESC, and carries discovery caveat', async () => {
    ...
    expect(read).toHaveBeenCalledWith('GetTickets', {
      clientid: 9001,
      limitnum: 10,
      limitstart: 0,
    });
    ...
    expect(Object.keys(p.items[0]).sort()).toEqual(
      ['date', 'deptname', 'lastreply', 'status', 'subject', 'ticketid', 'tid'].sort()
    );
  ...
  it('registers a stable outputSchema describing legacy + governed envelopes', () => {
    ...
    const shape = (cfg.outputSchema as { shape: Record<string, unknown> }).shape;
  ...
  it('list_client_orders maps GetOrders rows with userid and sorts by date DESC', async () => {
    ...
    expect(Object.keys(p.items[0]).sort()).toEqual(
      ['amount', 'date', 'invoiceid', 'name', 'orderid', 'ordernum', 'status'].sort()
    );
  });
});

...
// Read-only + production posture. Phase G+: production is sealed by the
// deny-by-default policy table (no absolute env block). With MCP_MODE
// read_only the FIRST failing gate is read_only_mode; production would ALSO
// be sealed at mode=full via the empty prod allowlist (proven in
// tests/write/executionGate.test.ts keystone). Either way: zero mutate.
...
describe('write-flow tools (read-only + production posture)', () => {
  it('registers the 10 flow tools', () => {
  ...
  it('unknown consumer is denied at draft (no intent)', async () => {
    const { handlers } = harness();
    const r = await handlers.draft_write_intent({
      scope: 'client_note:write',
      params: { clientid: 1, note: 'x' },
      naturalKey: 'k-unknown',
  ...
  it('FULL FLOW: execute is hard-blocked in production; WHMCS mutate NEVER called', async () => {
    ...
    // Phase G+: read_only is gate 2; it fires first in a read_only posture.
  ...
  it('execute without prior approval is blocked (intent_not_approved) — no mutate', async () => {
    ...
    const d = await handlers.draft_write_intent({
      scope: 'client_note:write',
      params: { clientid: 7, note: 'n' },
      naturalKey: 'flow-noappr',
  ...
  it('idempotency key is deterministic for same consumer+scope+naturalKey', async () => {
    const { handlers } = harness();
    const mk = () =>
      handlers.draft_write_intent({
        scope: 'client_note:write',
        params: { clientid: 1, note: 'n' },
        naturalKey: 'k-idem-stable',
        projected_effect: 'note',
        ...tok('writer'),
      });

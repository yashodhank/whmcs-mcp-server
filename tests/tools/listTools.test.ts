...
it('maps limit/offset→limitnum/limitstart and returns envelope', async () => {
  ...
  registerListTool(server as any, { read } as any, logger, rateLimiter, {
    ...
    normalizerPath: 'things',
    extraSchema: {},
    mapItem: (t: any) => ({ id: t.id }),
  });
...
it('passes fixedParams into the WHMCS call', async () => {
  ...
  const read = vi
    .fn()
    .mockResolvedValue({ totalresults: 0, numreturned: 0, startnumber: 0, xs: { x: [] } });
  registerListTool(server as any, { read } as any, logger, rateLimiter, {
    name: 'list_xs',
    description: 'd',
    action: 'GetXs',
  ...
  expect(read).toHaveBeenCalledWith('GetXs', {
    userid: 7,
    limitnum: 10,
    limitstart: 0,
    orderby: 'date',
...
it('decodeCursor is defensive: garbage / malformed / negative → 0, never throws', () => {
  ...
  expect(decodeCursor(Buffer.from(JSON.stringify({ offset: -5 }), 'utf8').toString('base64'))).toBe(
    0
  );
  expect(
    decodeCursor(Buffer.from(JSON.stringify({ offset: 'x' }), 'utf8').toString('base64'))
  ).toBe(0);
...
it('a garbage cursor is treated as offset 0 (first page), never crashes', async () => {
  ...
  const read = vi
    .fn()
    .mockResolvedValue({ totalresults: 1, numreturned: 1, startnumber: 0, ps: { p: [{ id: 1 }] } });
  registerListTool(server as any, { read } as any, logger, rateLimiter, {
    name: 'list_ps3',
    description: 'd',
    action: 'GetPs',
...
it('enforces client-mode scope', async () => {
  ...
  const { server, handlers, logger, rateLimiter } = harness();
  rlt(server as any, { read: vi.fn() } as any, logger, rateLimiter, {
    name: 'list_z',
    description: 'd',
    action: 'GetZ',
    clientParam: 'clientid',

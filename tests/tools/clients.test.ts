  ...
  describe('get_client_details', () => {
    it('should validate client IDs are positive integers', () => {
      ...
      const getClientDetailsSchema = z.object({
        ...
        (value) => value.clientid !== undefined || value.clientids !== undefined,
      ...
      expect(getClientDetailsSchema.safeParse({ clientid: [123, -1] }).success).toBe(false);
      expect(getClientDetailsSchema.safeParse({ clientids: [] }).success).toBe(false);
      expect(getClientDetailsSchema.safeParse({ clientids: [123, -1] }).success).toBe(false);

...
describe('mapToCanonicalTicket', () => {
  it('maps GetTicket with replies.reply + notes.note wrappers', () => {
    const raw = {
      ...
      replies: {
        reply: {
          '0': { replyid: 1, name: 'John Doe', email: 'john@example.com', message: 'still broken', date: '2026-03-02', admin: '' },
          '1': { replyid: 2, name: 'Agent', message: 'investigating', date: '2026-03-02', admin: 'agent1' },
        },
      },
      notes: { note: [{ noteid: 9, message: 'internal: escalate', admin: 'agent1', date: '2026-03-02' }] },
    };
  ...
  it('single reply object + garbage', () => {
    const c = mapToCanonicalTicket({ id: 2, replies: { reply: { replyid: 7, message: 'solo' } }, notes: {} });

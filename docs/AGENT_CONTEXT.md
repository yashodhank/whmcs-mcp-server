...
- **Reads**: `src/whmcs/actionPolicy.ts` (READ_ALLOWLIST + deny-by-default) →
  ...
  projection) → tools in `src/tools/{listTools,reportingListTools,aggregators,
  capabilityShellTools,clients,billing,domains,services,support}.ts`.
...
- **Writes (tiered governance)**: `src/write/{types,validation,paramMapping,
  ...
  Flow: draft → validate → approve → execute. Authorizer is deny-by-default.
...
## Tests / validation
- Targeted first; full suite (`npx vitest run`, currently 812 pass) only when

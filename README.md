...
- **~70 MCP tools** across several layers (see [Tool Catalog](#tool-catalog)):
  ...
  - **8.13.7 jobs** — `ops_ask` (staff digest / inbox / billing card / GDPR export) and `mcp_doctor` (OIDC + API-role + version). Audience from staff consumer/OIDC allow-lists. Customer door is OIDC link/handoff until a user-delegated API is proven. HTTP Grok uses a federation AS (`aud` = this MCP); WHMCS tokens are not MCP Bearers.
...
| WHMCS request pipeline              | [docs/design/whmcs-request-pipeline.md](docs/design/whmcs-request-pipeline.md)       |
...
| `MCP_TRANSPORT`                     | `stdio`                  | Transport: `stdio` or opt-in Streamable HTTP                                                                                                                                                                               |
| `MCP_PROTOCOL_RUNTIME`              | `v2`                     | Primary dual-era runtime; set `legacy` and restart/respawn only as a bounded rollback                                                                                                                                       |
| `MCP_HTTP_ALLOWED_HOSTS`            | loopback hosts           | Comma-separated Host-header allowlist without ports; public HTTP deployments must add their canonical hostname                                                                                                             |
| `MCP_HTTP_DRAIN_TIMEOUT_MS`         | `10000`                  | Maximum graceful-drain wait for in-flight modern HTTP requests                                                                                                                                                             |
| `MCP_READ_MAX_CONCURRENCY`          | `8`                      | Per-`WhmcsClient`/WHMCS-installation active-read ceiling (`1..64`); every read is scheduled, and queued reads are fair and abortable                                                                                          |
| `MCP_READ_COALESCE_ENABLED`         | `false`                  | Canary opt-in: join identical in-flight reads only for `MCP_READ_CACHE_ACTIONS`, separated by raw-data governance scope                                                                                                    |
| `MCP_READ_CACHE_TTL_MS`             | `0`                      | Completed-result cache TTL; zero keeps the process-local read cache off                                                                                                                                                     |

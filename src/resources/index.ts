...
export function registerResources(
  ...
  const readClientSummary = async (
    ...
    try {
      ...
      return governedResourceText({
        ...
        canonical: () => mapToCanonicalClient(client as unknown as Record<string, unknown>),
      });
    } catch (error) {
  ...
  const readInvoiceHistory = async (
    ...
    try {
      ...
      return governedResourceText({
        ...
        canonical: () => mapToCanonicalInvoice(invoice as unknown as Record<string, unknown>),
      });
    } catch (error) {
  ...
  const readTicketThread = async (
    ...
    try {
      ...
      return governedResourceText({
        uri: safeUri,
        legacy: payload,
        canonical: () => mapToCanonicalTicket(ticket as unknown as Record<string, unknown>),
      });
    } catch (error) {
              ...
              text: JSON.stringify(
                {
                  ...
                  tickets_note:
                    'best-effort; not guaranteed full support history (GetTickets clientid filter may miss operator/admin-created tickets)',
                },
                null,
                2
              ),
  ...
  // ============================================
  server.resource('system-activity', 'whmcs://system/activity', async (uri) => {
    const resourceLogger = logger.child();
    const safeUri = stripAuthFromUri(uri);
          ...
          contents: [
            {
              ...
              mimeType: 'application/json',
              text: JSON.stringify({
                error: 'System activity is not available in client access mode.',
              }),
            },
          ],
        };

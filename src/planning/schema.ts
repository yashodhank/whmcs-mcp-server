...
export const planIRSchema = z
  .object({
    ...
    goal: candidatePlanSchema.shape.goal,
    requested_outcome: candidatePlanSchema.shape.requested_outcome,
    assumptions: candidatePlanSchema.shape.assumptions,
    alternatives: candidatePlanSchema.shape.alternatives,
    selected_alternative_id: candidatePlanSchema.shape.selected_alternative_id,
    steps: z.array(compiledPlanStepSchema).max(100),
    execution_mode: candidatePlanSchema.shape.execution_mode,
    executable: z.literal(false),
    estimated_whmcs_calls: z.number().int().nonnegative(),
    expected_latency_class: z.enum(['instant', 'low', 'moderate', 'high']),
    catalog_version: z.number().int().positive(),

 ...
 *
 * Four original Phase-C actions are promoted governed reads after approved
 * evidence. `GetUsers` remains an external-only, unverified capability shell:
 * it never calls WHMCS and returns structured `capability_unavailable` data.
 * Every handler preserves the original public contract and never fabricates
 * support.
 */
...
import { registerCatalogOperation } from '../catalog/adapter.js';
import { createCapabilityShellCatalog } from '../catalog/packs/capabilityShell.js';

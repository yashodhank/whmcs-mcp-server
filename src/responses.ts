 ...
 */
export function success(data: unknown, message?: string, warnings?: string[]): McpToolResponse {
 ...
 */
export function error(errorMessage: string, code?: ErrorCode, details?: unknown): McpToolResponse {
 ...
 * @returns Formatted MCP tool error response
 */
export function modeRestricted(currentMode: string, requiredMode = 'full'): McpToolResponse {
  return error(
...
export function validationError(field: string, reason: string): McpToolResponse {
  return error(`Validation failed for '${field}': ${reason}`, ERROR_CODES.VALIDATION_FAILED, {
    field,
    reason,
  });
}
...
export function notFound(resourceType: string, resourceId: string | number): McpToolResponse {
  return error(`${resourceType} not found: ${resourceId}`, ERROR_CODES.RESOURCE_NOT_FOUND, {
    resourceType,
    resourceId,
  });
}
